...
char* indexes(const char* path)
{
    ...
    if(access("index.html", F_OK) != -1 ) {
        ...
        // malloc some memory to newpath 
        newpath = malloc(strlen(path) + strlen(indexhtml) + 1);
    
        // reconfig the path string
        strcpy(newpath, path);
        ...
        // free up the memory for newpath
        free(newpath);
        
    // otherwise, if index.php exists, do likewise
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // Store at the address in query the query substring from request-target & absolute-path at the address in abs_path
    char* abs_pathcopy;
    char* querycopy;
